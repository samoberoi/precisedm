...
const GestationPage = () => {
  ...
  const handleCalculate = async () => {
    const res = await calculate("gestation", form as unknown as Record<string, unknown>);
    if (res) {
  ...
  return (
    <div className={`min-h-screen bg-background ${isWebsite ? "py-10" : "pb-36"}`}>
      {isWebsite && <Seo page={getPageSeo("/gestation-tool")!} />}
      ...
      <div className={isWebsite ? cx : "px-5 pt-2"}>
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="relative z-10">
            <p className={`font-semibold text-white/60 uppercase tracking-widest ${isWebsite ? "text-xs" : "text-[10px]"}`}>Pregnancy Care</p>
            <h2 className={`font-extrabold text-white mt-1 ${isWebsite ? "text-2xl lg:text-3xl" : "text-lg"}`}>Gestation Calculator</h2>
            <p className={`text-white/70 mt-2 leading-snug ${isWebsite ? "text-sm max-w-md" : "text-[11px] max-w-[200px]"}`}>Calculate adjustments to Basal and Prandial insulin doses</p>
          </div>
          <img src={gestationIcon} alt="" className={`absolute -bottom-2 -right-2 opacity-15 object-contain ${isWebsite ? "h-32 w-32" : "h-24 w-24"}`} />
        </motion.div>
      </div>
              ...
              <motion.div key={step} custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit"
                ...
                {step === 1 && (
                  <div className="space-y-4">
                    ...
                    <FieldGroup label="Additional correction insulin dose per day (CTD), units">
